...
namespace algorithms {
    bool is_opening_brace(const char& c){
        ...
        return (std::end(opening_braces) != std::find(std::begin(opening_braces), std::end(opening_braces), c));
    }
    bool is_closing_brace(const char& c){
        std::vector<char> closing_braces = {'}',']',')'};
        return (std::end(closing_braces) != std::find(std::begin(closing_braces), std::end(closing_braces), c));
    }
